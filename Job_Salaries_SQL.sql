...
-- 4. Berapa rata-rata gaji data analyst, science dkk?
SELECT
	AVG(salary_in_usd) AS avg_salary_all
FROM
	ds_salaries;
    
-- 4. Berapa rata-rata gaji data analyst saja?
SELECT
	AVG(salary_in_usd) AS avg_salary_DA
FROM
	ds_salaries
WHERE
	job_title LIKE '%data analyst%';
    
    -- 4.1 Berapa rata-rata gaji data analyst, science dkk berdasarkan experience levelnya? (rupiah/bulan)
SELECT
	experience_level,
...
GROUP BY
    ...
    -- Revisi jawaban nomor 4.1 (data analyst saja)
SELECT
	experience_level,
	(AVG(salary_in_usd) * 15000) / 12 AS avg_sal_rp_monthly
FROM
...
order by 2 desc;
    ...
    -- 4.2 Berapa rata-rata gaji data analyst, science dkk berdasarkan experience level dan jenis employment?
SELECT
	experience_level,
	employment_type,
	(AVG(salary_in_usd) * 15000) / 12 AS avg_sal_rp_monthly
...
-- (untuk pekerjaan yang berkaitan dengan data analyst yang penuh waktu (full time)
-- menggunaka CTE
WITH ds_1 AS (
	SELECT
		work_year,
